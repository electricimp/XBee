class XBee {
    ...
    function _decodeATResponse(data) {
        ...
        data.seek(8, 'b');
        local len = (data[1] * 0xFF + data[2]) - 5;
    ...
    function _decodeZigbeeReceivePacket(data) {
        ...
        data.seek(15, 'b');
        local len = (data[1] * 0xFF + data[2]) - 12;
        if (len > 0) decode.data <- data.readblob(len);
        return decode;
    }
    ...
    function _decodeZigbeeTransmitStatus(data) {
        ...
        decode.cmdid <- data[3];
        decode.frameid <- data[4];
        decode.address16bit <- data[5] * 0xFF + data[6];
    ...
    function _decodeNodeIDIndicator(data) {
        ...
        decode.status.code <- data[14];
        decode.status.message <- _getPacketStatus(data[14]);
        decode.sourceAddress16bit <- data[15] * 0xFF + data[16];
        ...
        decode.parent16BitAddress <- data[offset] * 0xFF + data[offset + 1];
        decode.deviceType <- data[offset + 2];
        decode.sourceEvent <- data[offset + 3];
        decode.digiProfileID <- data[offset + 4] * 0xFF + data[offset + 5];
        decode.manufacturerID <- data[offset + 6] * 0xFF + data[offset + 7];
        return decode;
    }
    ...
    function _decodeRouteRecordIndicator(data) {
        ...
        for (local i = 0 ; i < data[15] ; ++i) {
            local a = data[16 + (i * 2)] * 0xFF + data[17 + (i * 2)];
    ...
    function _decodeXBeeSensorReadIndicator(data) {
        ...
        data.temp <- data[24] * 0xFF + data[25];
        return decode;
    }
    ...
    function _dataReceivedAPI() {
        ...
        if (_frameByteCount == 5) {
            ...
            if (_escaped) {
                ...
                _frameSize += _buffer[1] * 0xFF + _buffer[2];
            }
